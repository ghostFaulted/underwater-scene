...
void Model::Draw(Shader &shader, const AppState *appState) const {
    for (const auto &mesh: meshes) {
        ...
        // Apply runtime offsets from AppState if provided
        if (appState != nullptr) {
        ...
        // If mesh was attached to a bone during load, use attachmentTransform
        // (bone global in model space, without inverse-bind offset).
...
void Model::loadModel(const std::string &path) {
    ...
    // aiProcess_LimitBoneWeights caps influences to MAX_BONE_INFLUENCE (4) per
    // vertex, which is required by our Vertex layout and GPU skinning shader.
    ...
    // Post-processing: attach eye/teeth meshes to a head/jaw-like bone.
    // If no such bone exists, fall back to nearest bone by mesh centroid.
    ...
    // Post-processing: compute local offset for attached meshes so their centroid
    // aligns with the bone position (in model local space).
    for (auto &m: meshes) {
        ...
        // Find bone position in model space (from boneDebug)
        ...
        // Compute offset so that centroid + offset aligns with bone position
...
std::vector<Model::MeshDebugInfo> Model::GetMeshDebugInfo() const {
    ...
    for (const auto &m: meshes) {
        ...
        // Pivot translation from node transform (often zero in FBX exports).
        info.pivotTranslation = glm::vec3(m.nodeTransform[3]);

        // Geometric centroid / extents in mesh local space.
        if (!m.vertices.empty()) {
...
void Model::ApplyLocalRotationToMeshes(const std::string &namePattern, const glm::mat4 &localRotation) {
    ...
    for (auto &m: meshes) {
        ...
        if (n.find(patLower) != std::string::npos) {
            // Apply rotation in model-local space after existing node transform
