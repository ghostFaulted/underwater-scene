...
void SplinePath::BuildFrames() {
    ...
    for (std::size_t i = 0; i < sampleCount; ++i) {
        ...
        glm::vec3 binormal = glm::cross(tangent, worldUp);
        const float binormalLenSq = glm::dot(binormal, binormal);

        if (binormalLenSq < kEpsilon) {
            binormal = previousBinormal;
        }
        else {
            binormal = binormal * (1.0f / std::sqrt(binormalLenSq));
        }
...
void SplinePath::ValidateFrames() const {
    // ¬‡ÎË‰‡ˆËˇ ÓÚÍÎ˛˜ÂÌ‡ ‰Îˇ ·ÂÒÍÓÌÙÎËÍÚÌÓÈ Ò·ÓÍË
}
...
glm::vec3 SplinePath::CatmullRom(const glm::vec3 p0, const glm::vec3 p1, const glm::vec3 p2, const glm::vec3 p3, const float t) {
    const float t2 = t * t;
    const float t3 = t2 * t;
...
glm::vec3 SplinePath::CatmullRomTangent(const glm::vec3 p0, const glm::vec3 p1, const glm::vec3 p2, const glm::vec3 p3, const float t) {
    const float t2 = t * t;
